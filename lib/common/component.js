	...
	render(){
		return (
			<header className="mg-header mg-pos-r">
			    ...
			    <h1 className="mg-header-title mg-text-align mg-fs32">{this.props.headerTitle}</h1>
			    <div className="mg-header-r mg-pos-a">
			    	{this.props.search ? <a className="mg-header-search mg-bgs mg-text-hidden" href="">搜索</a> : ''}
			    	{this.props.myOrder ? <a className="mg-header-link mg-fs26" href="">我的订单</a> : ''}
			    	{this.props.homne ? <a className="mg-header-home mg-bgs mg-text-hidden" href="">首页</a> : ''}
			    </div>
			</header>
		)
	}
