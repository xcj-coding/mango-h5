...
class HOME extends Component{
    ...
    render(){
        ...
        return (
            <div className="mg-fadeInRight"> {/* mg-fadeInRight加载进入动画 */}
                {
                    this.state.LOADINGTIP ? ( /* this.state.LOADINGTIP判断是否载loading */
                        <LoadingTip />
                    ) : (
                        <div className="mg-fadeInRight">
                            <MgHeader data={{ /* 页头部分:headerTitle页头标题; gotoBack是否显示返回; headerArea是否显示当前城市; headerShare是否显示分享; headerCollect是否显示收藏; headerHome是否显示返回首页 */
                                gotoBack:true,
                                headerArea:true,
                                headerShare:false,
                                headerCollect:false,
                            ...
                            {
                                this.state.AJAXFAILED ? /* 判断请求数据失败时显示错误提示 */
                                <NoContent title="请求失败" content="数据请求超时或者错误" />
                                :
                                <div>
                                    <HOTPRODUCTS hotData={hotData} /> {/* 当季热门 */}
                                    <PRODUCTSLIST listDate={listData} /> {/* 产品列表 */}
                                </div> 
                            }
                        </div>
                    )
...
class HOTPRODUCTS extends Component{
    ...
    render(){
        let productsLIst=this.state.hotData.areaList; /* productsLIst当季热门数据 */
        return (
            <div>
                {	productsLIst ?
                    <section className="mg-hot">
                    	...
                    	<div className="mg-hot-swiper">
                            <div className="mg-hot-list cf">
                                ...
                                {
                                	productsLIst.map(function(data, index) {  /* 循环展示当季热门数据 */
                            			return 	<div className="mg-hot-box" key={index}>
                                                	<a className="mg-hot-img" href={"#/holiday/details/"+data.productId}>
                                                		<ImageBox imageUrl={data.productImageUrl} /> {/*图片组件，imageUrl图片地址*/}
                        ...
                        return  <section className="mg-productlist" key={index}>
                                    <div className="mg-productlist-t">
                                        ...
                                        { data.cityList ? 
                                            <ul className="mg-productlist-list1 mg-g">
                                                {
                                                    data.cityList.map(function(dataChild, indexChild) {  /* 产品列表上部分：地区展示 */
                                                        return 	<li className="mg-col mg-col-2" key={indexChild}>
                                                                    <a href={"#/holiday/list/"+data.areaType+"/"+dataChild.cityId}>
                                                                        <ImageBox imageUrl={dataChild.cityPicture} /> {/*图片组件，imageUrl图片地址*/}
                                    ...
                                    { data.areaList ?
                                        <div className="mg-productlist-b">
                                            <ListItem areaList={data.areaList} />  { /* 产品列表下部分：产品展示 */}
                                        </div> : ""
                                    }
                                </section>;
                    })	
