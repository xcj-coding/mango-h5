...
class LIST extends Component{
	...
	componentWillMount(){
        ...
        getDataFromAPI({
            ...
            data:'{"clientIp":"127.0.0.1","page":"1","pageSize":"10"}',
    ...
    PriceToggle(){
        this.state.price ? this.setState({price:false,mask:false}) : this.setState({price:true,mask:true});
    }
    sortToggle(){
        this.state.sort ? this.setState({sort:false,mask:false}) : this.setState({sort:true,mask:true});
    }
    selectToggle(){
	...
	render(){
        ...
        let itemData = this.state.listData.data,itemContent,footer,price,sort,select,mask;
            ...
            itemContent =   <section className="mg-productlist">
                                <div className="mg-productlist-b">
                                    <ul className="mg-productlist-list2">
                                        {
                                            itemData.map((item,index) => <ListItem key={index} {...item}  />)
                                        }
                                    </ul>
                                </div>
                            </section>;
        ...
        if(this.state.mask){
            mask = <Mask  maskHandler={this.maskHandler.bind(this)} {...this.state}/>
        }
        if(this.state.price){
            price = <ListPrice priceToggle={this.PriceToggle.bind(this)} {...this.state} />
        }
        if(this.state.sort){
...
class ListPrice extends Component{
    ...
    render(){
        let priceArr = ['不限','¥1000以下','¥1000-¥2000','¥2001-¥3000','¥3001-¥5000','¥5000以上'];
        console.log(this.props.price);
        return (
            <section className={
            classnames('mg-tips-price',{
              'mg-tips-up': this.props.price
            })}
            >
                <h4>价格区间 <span>清除全部</span> </h4>
                <div className="mg-tips-price-content">
                    {
                        priceArr.map((item,index) => <ListA key={index}  onListA={this.onListA.bind(this)} txt={item} />)
                    }
                </div>
...
class ListA extends Component{
    ...
    render(){
        return (
            <a href="javascript:;" onClick={this.props.onListA} >{this.props.txt}</a>
        )
    }
}
...
class Mask extends Component{
    render(){
        return (
            <div className={
                classnames('mg-mark-black',{
                  'mg-mask-opin': this.props.mask
                })} 
